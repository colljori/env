...
alias cs_add_obs_ip='ip a show dev tun0 | grep -Eo 12[1-9] | xargs printf "ip a add 10.65.201.%d/24 dev eth1" | xargs ssh -F $SSH_CONFIG_FILE edgerouter'
alias cs_del_obs_ip='ip a show dev tun0 | grep -Eo 12[1-9] | xargs printf "ip a del 10.65.201.%d/24 dev eth1" | xargs ssh -F $SSH_CONFIG_FILE edgerouter'
